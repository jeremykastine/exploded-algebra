...
assert(/\.builder-keypad-panel \.builder-variable-button \{ top: 0; height: 25%; \}/.test(playerHtml), "x must occupy the top cell of the rightmost column");
assert(/\.builder-keypad-panel \.builder-prod-button \{ top: 25%; height: 37\.5%; \}/.test(playerHtml) && /\.builder-keypad-panel \.builder-sum-button \{ top: 62\.5%; height: 37\.5%; \}/.test(playerHtml), "Multiplication and addition must each occupy one and a half rows below x");
assert(playerHtml.includes('.builder-keypad-panel .builder-inv-button { grid-column: 5; grid-row: 4; }') && !playerHtml.includes('.builder-move-up-button'), "Inverse must replace x in the shifted number pad");
assert(playerHtml.includes('[data-builder-action="digit"][data-value="7"] { grid-column: 3; grid-row: 1; }') && playerHtml.includes('[data-builder-action="digit"][data-value="9"] { grid-column: 5; grid-row: 1; }'), "The number pad must occupy the three columns immediately left of the operation column");
assert(playerHtml.includes('M500 250 H599') && playerHtml.includes('M200 200 H500'), "The Builder grid overlay must show the one-and-a-half-row operation buttons without drawing number-pad lines through them");
...
assert(playerHtml.includes('body.left-handed .builder-keypad-panel .builder-sum-button { right: auto; left: 0; }') && playerHtml.includes('body.left-handed .builder-keypad-panel .builder-undo-button { grid-column: 5; grid-row: 1 / span 2; }') && playerHtml.includes('body.left-handed .builder-keypad-panel .builder-submit-button { grid-column: 5; grid-row: 3 / span 2; }') && playerHtml.includes('body.left-handed .builder-keypad-panel .builder-review-button { grid-column: 6; grid-row: 1; }'), "Left-handed mode must mirror the rightmost operation stack while preserving the unchanged control columns");
...
assert(!playerJs.includes("implicitOperation") && !playerJs.includes("autoMultiplyAfterNegativeOne") && !playerJs.includes("needsImplicitProduct"), "Expression Builder must not retain implicit operation insertion");
assert(/function enterDirectBuilderValue[\s\S]*?if \(!isBuilderPlaceholder\(current\)\)[\s\S]*?Choose an operation before entering another value/.test(playerJs), "x and negative one after a value must require an explicit operation");
assert(/function enterDirectBuilderInverse[\s\S]*?if \(!isBuilderPlaceholder\(current\)\)[\s\S]*?Choose an operation before entering an inverse/.test(playerJs), "An inverse after a value must require an explicit operation");
