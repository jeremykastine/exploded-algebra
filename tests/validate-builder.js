...
assert(playerHtml.includes('.builder-keypad-panel .builder-zoom-in-button { grid-column: 2; grid-row: 2; }') && playerHtml.includes('.builder-keypad-panel .builder-reset-view-button { grid-column: 2; grid-row: 4; }'), "Builder zoom and reset controls must sit below x");
...
assert(/\.builder-keypad-panel \{[\s\S]*?grid-template-columns: repeat\(6, minmax\(0, 1fr\)\);[\s\S]*?grid-template-rows: repeat\(4, minmax\(0, 1fr\)\);[\s\S]*?width: 100%;[\s\S]*?height: min\(var\(--bottom-panel-height\), 33\.333dvh\);/.test(playerHtml), "The Builder keypad must fill the entire resizable six-by-four controls panel");
