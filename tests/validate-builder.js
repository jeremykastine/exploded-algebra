...
assert(/\.bottom-controls-panel > \.builder-keypad-panel,[\s\S]*?\.bottom-controls-panel > \.exercise-completion-panel \{[\s\S]*?grid-template-columns: subgrid;[\s\S]*?grid-template-rows: subgrid;[\s\S]*?width: 100%;[\s\S]*?height: 100%;/.test(playerHtml), "The Builder keypad must inherit the entire resizable panel's six-by-four grid");
