...
assert(rendererJs.includes('const lastDigit = String(child.value).slice(-1)') && rendererJs.includes('!box.groupable'), "Builder must highlight the newest digit or pending operation rather than a future landing position");
...
assert(nearlyEqual(activeNumberOutline.x + activeNumberOutline.width, activeNumber.right() + activeNumberSequence.layout.builderItemPadding), "The active number outline must end immediately after the entered number");
...
assert(nearlyEqual(dangling.layout.x + dangling.layout.width, danglingOperator.x + danglingOperator.width + dangling.layout.builderItemPadding), "A dangling operation must end with normal item padding and no extra value space");
