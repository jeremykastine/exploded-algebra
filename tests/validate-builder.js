...
assert(playerHtml.includes('data-workspace-action="authoringCheckpoint"') && playerHtml.indexOf('data-workspace-action="authoringCheckpoint"') < playerHtml.indexOf('class="workspace-tool-group workspace-selection-group"'), "Record Step must be the first control in the preselection toolbar");
assert(playerHtml.includes('body.authoring-recording-session .workspace-toolbar [data-workspace-action="authoringCheckpoint"] { grid-column: 1; grid-row: 3; }'), "Record Step must occupy the far-left position in the bottom preselection row");
...
assert(builderJs.includes("function recordCurrentStep(snapshot, api)") && builderJs.includes("actionStartIndex") && builderJs.includes("actionEndIndex: actions.length"), "Record Step must save an explicit expression checkpoint and its action boundary");
assert(builderJs.includes("api.setRecordingCheckpointState(currentStepIsRecorded(fullSnapshot), true)"), "A newly recorded current step must turn Record Step into All Done");
assert(playerJs.includes("setRecordingCheckpointState(recorded, visible = true)") && playerJs.includes('notifyAuthoringHost("recording-checkpoint")'), "The embedded preselection control must synchronize with and notify the Exercise Builder");
assert(builderJs.includes("snapshot.recorder ? snapshot : getApi()?.getSnapshot() || snapshot"), "Interaction-only messages must not erase recorded checkpoints");
assert(builderJs.includes("reconcileRecordedSteps(snapshot)") && builderJs.includes("candidate.actionPrefix === getActionPrefix"), "Checkpoints discarded by an unrecorded undo must be removed from the saved path");
...
assert(playerHtml.includes('.builder-keypad-panel .builder-variable-button { grid-column: 7; grid-row: 2; }'), "The x button must sit above 9");
assert(playerHtml.includes('.builder-keypad-panel .builder-inv-button { grid-column: 4; grid-row: 3; }') && playerHtml.includes('.builder-keypad-panel .builder-exit-inv-button { grid-column: 4; grid-row: 4; }'), "Inverse and Exit Inverse must top the column beside the number pad");
assert(playerHtml.includes('.builder-keypad-panel .builder-sum-button { grid-column: 4; grid-row: 6; }') && playerHtml.includes('.builder-keypad-panel .builder-prod-button { grid-column: 4; grid-row: 5; }'), "Addition and multiplication must bottom the column beside the number pad");
assert(playerHtml.includes('button.builder-submit-button {\n            grid-column: 2;\n            grid-row: 5;') && playerHtml.includes('button.builder-review-button {\n            grid-column: 2;\n            grid-row: 6;'), "Submit and Review must form the outer control column");
assert(playerHtml.includes('.builder-keypad-panel .builder-pan-button { grid-column: 3; grid-row: 3; }') && playerHtml.includes('.builder-keypad-panel .builder-reset-view-button { grid-column: 3; grid-row: 6; }'), "Builder pan, zoom, and reset controls must form the middle control column");
...
assert(playerHtml.includes('body.left-handed .builder-keypad-panel .builder-variable-button { grid-column: 3; grid-row: 2; }') && playerHtml.includes('body.left-handed .builder-keypad-panel .builder-submit-button { grid-column: 6; grid-row: 5; }') && playerHtml.includes('body.left-handed .builder-keypad-panel .builder-pan-button { grid-column: 5; grid-row: 3; }'), "Left-handed mode must mirror the complete keypad cluster");
assert(playerJs.includes('data-builder-view-action="pan"') && playerJs.includes('data-builder-view-action="zoomIn"') && playerJs.includes('data-builder-view-action="zoomOut"') && playerJs.includes('data-builder-view-action="resetZoom"'), "Integrated Expression Builder must provide pan, zoom, and reset controls");
