...
assert(/function enterDirectBuilderValue[\s\S]*?String\(value\) === "-1"[\s\S]*?"sum"[\s\S]*?String\(value\) === "x"[\s\S]*?"prod"/.test(playerJs), "Expression Builder must insert addition before negative one and multiplication before x");
