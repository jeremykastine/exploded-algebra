...
assert(builderHtml.includes('id="finishRecordingButton" type="button" class="record-step-button" hidden>Record Step</button>'), "Phase 3 must start with a Record Step control");
assert(builderCss.includes("bottom: max(12px, env(safe-area-inset-bottom))") && builderCss.includes("left: max(12px, env(safe-area-inset-left))"), "The Phase 3 step control must occupy the lower-left corner");
...
assert(builderJs.includes('button.textContent = currentStepIsRecorded(fullSnapshot) ? "All Done" : "Record Step"'), "A newly recorded current step must turn Record Step into All Done");
...
assert(builderJs.includes("function syncFinishRecordingButton(snapshot)") && builderJs.includes("snapshot.preselectionActive !== true") && playerJs.includes('notifyAuthoringHost("interaction-state"'), "The Phase 3 step control must appear only while the workspace is in preselection");
